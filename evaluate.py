...
def plot_evaluation_results():
    eval_file = "results/eval_results.txt"  # Ensure this path is correct

    # Define categories to extract
    categories = ["bbox_2d", "AOS", "bbox_bev", "bbox_3d"]
    metrics = {cat: [] for cat in categories}
    ...
    for line in lines:
        ...
        for category in categories:
            if category.upper() in line:
                current_metric = category
        if "AP:" in line and current_metric:
            try:
                values = list(map(float, line.split(":")[1].strip().split()))
                metrics[current_metric] = values
            except ValueError:
                print(f"Warning: Could not parse values for {current_metric}. Skipping.")
                metrics[current_metric] = [0, 0, 0]  # Default values to avoid errors

    # Create the plots
    plt.figure(figsize=(10, 6))
    x_labels = ["Easy", "Moderate", "Hard"]

    for i, (metric, values) in enumerate(metrics.items()):
        if not values:  # Ensure non-empty values
            continue
        plt.subplot(2, 2, i + 1)
        plt.bar(x_labels, values, color=["blue", "orange", "green"])
        plt.ylim(0, 100)
        plt.title(f"{metric.upper()} AP")
        plt.xlabel("Difficulty")
        plt.ylabel("AP (%)")

    plt.tight_layout()
    plt.savefig("results/evaluation_plot.png")  # Save the figure
    plt.show()
